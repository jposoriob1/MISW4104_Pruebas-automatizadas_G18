  ...
  (async () => {
    ...
    for(b of browsers){
      ...
      //Web application ripping
       let elementLogin = [];
       await page.goto(baseUrl, { waitUntil: "networkidle" }).catch((err) => {
        console.log(err);
        return;
              });
        await getTextInputs (page, elementLogin);
        await getButtons (page, elementLogin);
        elementLogin.splice (2, 1);
        await interactWithObjects (elementLogin, page, 0, "");
